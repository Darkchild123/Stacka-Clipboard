...
tokens, and other secrets people routinely copy. ClipDrop is built around that
risk.

### No network attack surface

ClipDrop is **entirely local**. It runs no server, opens no ports, and has no
...
ever unavailable, ClipDrop stores the data unencrypted and says so, rather than
losing it.
...
| Mode | Location |
|---|---|
| Packaged app | `%APPDATA%\ClipDrop\` (per-user, outside `Program Files`) |
...
no need to touch ClipDrop at all. No local clipboard tool can prevent that. The
